/*
 * @Author: yangyuan
 * @Date: 2020-04-25 00:45:25
 * @Email: 1367511704@qq.com
 * @LastEditTime: 2020-04-25 01:23:28
 * @Description:
 */
...
//文件夹目录
const distDir = path.resolve(__dirname, "../sh_msa"); //待打包
const distZipPath = path.resolve(__dirname, `../sh_msa.zip`); //打包后地址(dist.zip是文件名,不需要更改, 主要在config中配置 PATH 即可)
...
const uploadZipBySSH = async () => {
    ...
    loading.spinner = spinner_style.arrow4;
    try {
        await SSH.putFiles([{ local: distZipPath, remote: config.PATH + "/sh_msa.zip" }]); //local 本地 ; remote 服务器 ;
        successLog("上传成功!");
        loading.text = "正在解压文件";
        await runCommand("unzip ./sh_msa.zip"); //解压
        await runCommand(`rm -rf ${config.PATH}/sh_msa.zip`); //解压完删除线上压缩包
        ...
        await runCommand(`mv -f ${config.PATH}/sh_msa/*  ${config.PATH}`);
        await runCommand(`rm -rf ${config.PATH}/sh_msa`); //移出后删除 dist 文件夹
        SSH.dispose(); //断开连接
    } catch (error) {
        errorLog(error);
        errorLog("上传失败!");
